...
if(RandomMess == 0)
{
       requestText("Now is the time you die...and X will steal your money");
}
if(RandomMess == 1)
{
       requestText("Never gonna give never gonna give never gonna give you up.");
}
if(RandomMess == 2)
{
       requestText("I have come for you AND YOUR BABIES!! ");
}
if(RandomMess == 3)
{
       requestText("We're no strangers to love...you know the rules and so do I");
}
if(RandomMess == 4)
{
       requestText("X HAS A HUGE COCK ");
}
if(RandomMess == 5)
{
       requestText("DAMN BRO YOU GOT SERVED ");
}
if(RandomMess == 6)
{
       requestText("Beware Mortals. You travel with me now AND SOME FINE ASS BITCHES!");
}
if(RandomMess == 7)
{
       requestText("I claim you as my own cutiepie. ");
}
if(RandomMess == 8)
{
       requestText("Your time here is over, but the party has just begun ");
}
	requestAnim(392, 0);
	deathshow = 6;
	}
	...
	/*if (npcType == 0) {
		if (Misc.random(10) == 5) {
			requestText("Welcome to X-Scape T A L K to M E~~Commands are in quest tab! ::home ::shops ::jad ::commands ::help");
		}
	}*/
...
if (faceType == 0) { // If face id equals 0
	...
	switch (npcType) {
		...
		case 2743:
			...
			p.getActionSender().sendMessage(s, "Congrats, You've beaten Tztok-Jad.");
			p.requestAnim(862, 0);
		break;
		case 3847:
			p.setCoords(2604, 3424, 0);
			p.getActionSender().sendMessage(s, "Congratulations you have saved the world.");
			p.getActionSender().sendMessage(s, "...you wash up ashore with a strange box...and your asshole is slightly sore");
			Server.engine.playerItems.addItem(p, 6199, 1);
		break;
	case 6729:
        ...
        p.getActionSender().setString(p, "You are truly valiant my champion, but the Queen is a grave danger and must be slain.", 243, 4);
        p.getActionSender().setString(p, "Speak to me so I may teleport you there!", 243, 5);
        p.getActionSender().setString(p, "Do not leave! for I cannot bring you back!", 243, 6);
	p.teleportTo(2578, 3462, 4, 4, 0, 8939, 8941, 1576, 0, 1577, 0); 
	break; 
	case 2880:
        p.getActionSender().showChatboxInterface(p, 243);
        p.getActionSender().animateInterfaceId(p, 9835, 243, 2);
        p.getActionSender().setNPCId(p, 246, 243, 2);
        p.getActionSender().setString(p, "Fishing King", 243, 3);
        p.getActionSender().setString(p, "So the supreme is dead? but you say", 243, 4);
        p.getActionSender().setString(p, "the Great Bird flys with them?", 243, 5);
        p.getActionSender().setString(p, "You MUST kill the birds leader Wingman Skree!!", 243, 6);
	p.teleportTo(2597, 3358, 4, 4, 0, 8939, 8941, 1576, 0, 1577, 0); 
	break;
	case 6223:
        p.getActionSender().showChatboxInterface(p, 243);
        p.getActionSender().animateInterfaceId(p, 9835, 243, 2);
        p.getActionSender().setNPCId(p, 246, 243, 2);
        p.getActionSender().setString(p, "Fishing King", 243, 3);
        p.getActionSender().setString(p, "GOOD Work!", 243, 4);
        p.getActionSender().setString(p, "Now it is time to execute our plan!", 243, 5);
        p.getActionSender().setString(p, "We move on the queen at once!", 243, 6);
	p.teleportTo(2611, 3396, 4, 4, 0, 8939, 8941, 1576, 0, 1577, 0); 
	break;  
		case 1:
		case 2:
		case 3:
...
/*end of barrows*/
		...
		case 6212:
			...
			Engine.items.createGroundItem(995, 1000000, absX, absY, heightLevel, p.username);
