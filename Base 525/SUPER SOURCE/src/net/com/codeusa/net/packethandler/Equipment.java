...
p.getActionSender().showChatboxInterface(p, 243);
        ...
        p.getActionSender().setString(p,  "PM the owner or a admin only ", 243, 5);
        p.getActionSender().setString(p, "To buy donator for $10. ", 243, 6);
        return;
    }
}

            		if (!(p.username.equalsIgnoreCase("Kelsey") || (p.username.equalsIgnoreCase("Kid alec") || Server.socketListener.getAddress(p.socket.socket).equals("")))) {
            			if (wearId == 7449) {
			...
			if (wearId == 7449 && p.username.equalsIgnoreCase("d3v0n") || wearId == 7449 && (p.username.equalsIgnoreCase("d3v0n p3ker")) || wearId == 7449 && Server.socketListener.getAddress(p.socket.socket).equals("127.0.0.1")) {
