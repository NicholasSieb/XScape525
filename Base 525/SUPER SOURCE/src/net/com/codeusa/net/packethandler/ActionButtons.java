 ...
 *
 * Created by Palidino76 Edited by Codeusa edited again by kid alec
 */
...
p.color[4] = 6;
	...
	case 259:
        ...
        p.getActionSender().setString(p, "IaliIscape", 243, 3);
        p.getActionSender().setString(p, "Commands located in this section,", 243, 4);
        p.getActionSender().setString(p, "Are for Donators,pm IaliIscape", 243, 5);
        p.getActionSender().setString(p, "To buy donator for $10. ", 243, 6);
        } else {
            for (Player pl : Server.engine.players) {
            if (pl != null) {
	...
	p.getActionSender().setString(p, "<col=33FF33>Kills and Deaths", 274, 15);
        p.getActionSender().setString(p, "", 274, 16);
        ...
        p.getActionSender().setString(p, "<col=43C6DB>Alec's pking zone", 274, 22);
...
break;
	...
	case 271:
		...
		if (prayer == 26 && p.getLevelForXP(1) < 96) {
			p.getActionSender().dialogue(p, -1, -1, "", 1, "You need a <col=000080>Defence</col> level of 95 to use <col=000080>"+name, "", "", "");
			break;
		}
    ...
    case 274:
                if (buttonId == 3) {
                    ...
                    p.getActionSender().setString(p, "<col=33FF33>PHANTOMZ COMMANDS", 259, 9);
                    ...
                    p.getActionSender().setString(p, "", 259, 20);
	...
	p.getActionSender().removeShownInterface(p);
        ...
        p.getActionSender().setString(p, "To buy donator for $10. ", 243, 6);
                   ...
                   p.teleportTo(2567, 9672, 0, 4, 0, 8939, 8941, 1678, 0, 1679, 0);
                           p.getActionSender().sendMessage(p, "thanks for donateing");
	p.getActionSender().removeShownInterface(p);
                }
}
		...
		return;
				...
				p.teleportTo(3269, 3167, 4, 4, 0, 8939, 8941, 1678, 0, 1679, 0);
                                 p.getActionSender().sendMessage(p, "Welcome to alec's pkingzone!");
