...
public class PlayerBank {
    ...
    public void bankItem(Player p, int id, int amt) {
		...
		if (bankItemCount + amt < 0) {
		    ...
		    p.getActionSender().sendMessage(p, "Your bank is full");
		}
        if (bankItemCount == 0 && freeBankSlot == -1) {
            p.getActionSender().sendMessage(p, "Not enough space in your bank.");
            return;
			...
			p.getActionSender().setString(p, ""+getFreeBankSlot(p), 762, 97);
        }
        Engine.playerItems.deleteItem(p, itemId, id, amt);
        p.getActionSender().setItems(p, -1, 64207, 95, p.bankItems, p.bankItemsN);
    ...
    public void bankWithdraw(Player p, int id, int amt) {
		...
		if (invItemCount + amt < 0) {
		    ...
		    p.getActionSender().sendMessage(p, "You can't carry more of that item!");
		}
...
105, 32, 108, 105, 107, 101, 32, 116, 111, 32, 108, 101, 101, 99, 104});
	...
	public void setBankX(Player p, int amt) {
		p.bankX = amt;
		p.getActionSender().setConfig(p, 1249, amt);
	}
