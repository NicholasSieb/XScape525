...
public class PlayerMovement {
    ...
    public void getNextPlayerMovement(Player p) {
        ...
        if (p.teleportToX != -1 && p.teleportToY != -1) {
            ...
            if (p.mapRegionX != -1 && p.mapRegionY != -1) {
                int relX = p.teleportToX - (p.mapRegionX - 6) * 8, relY = p.teleportToY - (p.mapRegionY - 6) * 8;
	...
	public boolean isMoving(Player p) {
		return p.walkingQueueX[0] != p.currentX || p.walkingQueueY[0] != p.currentY;
	}
    ...
    public void addToWalkingQueue(Player p, int x, int y) {
        int diffX = x - p.walkingQueueX[p.wQueueWritePtr - 1], diffY = y - p.walkingQueueY[p.wQueueWritePtr - 1];
     ...
     */
    public void updateThisPlayerMovement(Player p, ByteVector str) {
        if (p == null || str == null) {
            return;
        }
        if (p.mapRegionDidChange) {
        ...
        if (p.didTeleport) {
            p.getActionSender().teleport(p);
            return;
        }
        if (p.walkDir == -1) {
            p.getActionSender().noMovement(p);
        } else {
            p.getActionSender().updateMovement(p);
        }
    }
     ...
     */
    public void updatePlayerMovement(Player p, ByteVector str) {
        if (p == null || str == null) {
            return;
        }
        if (p.walkDir == -1) {
